  ...
  await userexchangeschema
    ...
    .then((requests) => {
      ...
      if (data.length <= 0) {
        ...
        res.json({ status: 500, message: "No Exchange Requests found" });
      } else {
        res.json({
          status: 200,
          msg: "ok",
...
const displayExchangeRequest = async (req, res) => {
  try {
    const requests = await userexchangeschema
      .find({})
      .populate("userid")
      .populate("statusChangedBy");

    const filteredRequests = requests.filter(
      (request) => request.statusChangedBy !== null
    );

    if (filteredRequests.length === 0) {
      return res
        .status(404)
        .json({ message: "No requests found for this book" });
    }
